...
type ActiveClimbRow = Pick<
  Tables<'climbs'>,
  'attempts' | 'color_band' | 'flash' | 'id' | 'order_index' | 'sent'
>;
...
interface ActiveBoulderSessionData {
...
interface ActiveBoulderSessionStepProps {
  onDiscarded: () => void;
...
function isTouched(climb: ActiveClimbRow) {
  return climb.sent || climb.attempts > 0;
}

async function pauseActiveBoulderSession(session: ActiveSessionRow) {
...
async function toggleActiveBoulderSessionTimer(session: ActiveSessionRow) {
  ...
  await pauseActiveBoulderSession(session);
}

export default function ActiveBoulderSessionStep({
  onDiscarded,
  onFinishRequested,
  sessionId,
}: ActiveBoulderSessionStepProps) {
  const { user } = useAuth();
  ...
  const activeSessionQuery = useQuery({
    queryKey: ['active-boulder-session', sessionId],
    queryFn: async () => {
      const [sessionResult, climbsResult, attachmentsResult] = await Promise.all([
        supabase
          .from('sessions')
          .select(
            'id, date, gym_id, status, started_at, paused_at, paused_ms, planned_session_id, created_at, gyms(name, city)',
          )
          .eq('id', sessionId)
          .maybeSingle(),
        supabase
          .from('climbs')
          .select('id, color_band, sent, flash, attempts, order_index')
      ...
      return {
        ...
        session: sessionResult.data as ActiveBoulderSessionData['session'],
      } satisfies ActiveBoulderSessionData;
    },
    enabled: !!sessionId,
  });
  ...
  const touchedCount = activeSessionQuery.data?.climbs.filter(isTouched).length || 0;
  const completedCount = activeSessionQuery.data?.climbs.filter((climb) => climb.sent).length || 0;
  const triedCount =
    activeSessionQuery.data?.climbs.filter((climb) => !climb.sent && climb.attempts > 0).length || 0;
  const totalCount = activeSessionQuery.data?.climbs.length || 0;
  const progress = totalCount > 0 ? (touchedCount / totalCount) * 100 : 0;
  ...
  const updateClimb = useMutation({
    ...
    onError: (error) => {
      toast.error('No se pudo actualizar el bloque: ' + error.message);
    },
  });
  ...
  const toggleTimer = useMutation({
    mutationFn: async () => {
      if (!session) return;
      await toggleActiveBoulderSessionTimer(session);
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['active-boulder-session', sessionId] });
    },
    ...
    async (file: File, climbId: string) => {
      ...
      try {
        ...
        if (result.uploaded > 0 && result.failed === 0) {
          toast.success('Foto añadida al bloque');
        } else {
          toast('Foto en cola', {
  ...
  const openMobileCamera = useCallback(async (climbId: string) => {
  ...
  const handlePhotoRequest = (climbId: string) => {
    const shouldUseMobileCamera = isMobile || window.matchMedia('(max-width: 767px)').matches;
  ...
  const captureMobilePhoto = async () => {
    ...
    try {
      ...
      const fileName = `boulder-${new Date().toISOString().replace(/[:.]/g, '-')}.jpg`;
  ...
  const handleFinish = async () => {
    ...
    if (touchedCount === 0) {
      toast.error('Registra al menos un bloque o descarta la sesión.');
      return;
    }
    ...
    <div className="space-y-4">
      ...
      >
        <DialogContent className="max-w-md overflow-hidden p-0">
          <DialogHeader className="px-4 pt-4">
            ...
            <DialogDescription>
              En móvil se abre la cámara directamente para guardar la foto del bloque.
            </DialogDescription>
          </DialogHeader>
      ...
      <Card className="card-elevated border-primary/25">
        <CardHeader className="p-4 pb-3">
          <div className="flex items-start justify-between gap-3">
            <div>
              <CardTitle className="text-xl">Sesión activa de bloque</CardTitle>
              <p className="mt-1 text-sm text-muted-foreground">
                Marca cada bloque como top o intentado y añade foto si quieres.
              </p>
            </div>
          ...
          <div>
            <div className="mb-2 flex items-center justify-between text-sm">
              ...
              <span className="font-medium">
                {touchedCount}/{totalCount} registrados
              </span>
            </div>
            <Progress value={progress} />
            <div className="mt-2 flex flex-wrap gap-2 text-xs text-muted-foreground">
              <span>{completedCount} tops</span>
              <span>·</span>
              <span>{triedCount} intentados</span>
            </div>
          </div>
      ...
      <div className="space-y-3">
        {activeSessionQuery.data.climbs.map((climb, index) => {
          ...
          const touched = isTouched(climb);
          const colorBand = climb.color_band as BoulderColorBand | null;
          const colorOption =
            BOULDER_COLOR_OPTIONS.find((option) => option.value === colorBand) || null;
          const stateLabel = climb.sent ? 'Top' : touched ? 'Intentado' : 'Pendiente';
          ...
          return (
            <Card
              ...
              className={
                climb.sent
                  ? 'card-elevated border-green-500/30'
                  : touched
                    ? 'card-elevated border-amber-500/30'
                    : 'card-elevated'
              }
            >
              <CardContent className="space-y-4 py-4">
                <div className="flex items-start justify-between gap-3">
                  <div>
                    <div className="flex items-center gap-2">
                      ...
                      <span
                        className={`h-3.5 w-3.5 rounded-full ${colorOption?.className || 'bg-muted'}`}
                      />
                      <span className="text-lg font-semibold">{getBoulderColorLabel(colorBand)}</span>
                    </div>
                  ...
                  <div className="flex flex-wrap gap-2">
                    <Button
                      type="button"
                      size="sm"
                      variant={climb.sent ? 'default' : 'outline'}
                      onClick={() =>
                        updateClimb.mutate({
                          attempts: Math.max(1, climb.attempts || 1),
                          climbId: climb.id,
                          sent: true,
                        })
                      }
                    ...
                    <Button
                      type="button"
                      size="sm"
                      variant={!climb.sent && touched ? 'default' : 'outline'}
                      onClick={() =>
                        updateClimb.mutate({
                          attempts: Math.max(1, climb.attempts || 1),
                          climbId: climb.id,
                ...
                <div className="flex items-center justify-between rounded-lg bg-muted/40 px-3 py-3">
                  <div>
                    ...
                    <p className="text-xs text-muted-foreground">
                      {climb.sent && climb.attempts === 1
                        ? 'Flash'
                        : touched
                          ? 'Puedes bajar a 0 para dejarlo pendiente'
                          : 'Aún sin registrar'}
                    </p>
                  </div>
                  <div className="flex items-center gap-3">
                    <Button
                      ...
                      onClick={() =>
                        updateClimb.mutate({
                          attempts: Math.max(0, climb.attempts - 1),
                          climbId: climb.id,
                          sent: climb.sent && climb.attempts - 1 > 0,
                        })
                      }
                      disabled={updateClimb.isPending || climb.attempts <= 0}
                    >
                      <Minus className="h-4 w-4" />
                    </Button>
                    <span className="w-10 text-center text-xl font-bold">{climb.attempts}</span>
                    <Button
                      type="button"
                      variant="outline"
                      size="icon"
                ...
                <div className="flex items-center justify-between gap-3">
                  ...
                  <Button
                    ...
                    disabled={!touched || uploadingClimbId === climb.id}
                  >
                    <Camera className="mr-2 h-4 w-4" />
